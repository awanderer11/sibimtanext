...
const Proposal = () => {
  ...
  useEffect(() => {
    ...
    async function fetch() {
      db.collection(`data-mahasiswa/${router.query.nim}/proposal`)
        .where("nip", "==", nip)
        .onSnapshot((docs) => {
          const data: any[] = [];
          docs.forEach((it) => {
  ...
  const onSubmit = async () => {
    ...
    await db
      .doc(`data-mahasiswa/${router.query.nim}/proposal/${id}`)
      .set({
        topikBahasan: valMessage,
                ...
                <Tr key={id}>
                  ...
                  <Td>
                    <IconButton
                      ...
                      onClick={() =>
                        router.push(
                          `/mahasiswa/proposal/topik/${state.nim}/${it.id}`
                        )
                      }
                    />
                  </Td>
